...
Texture3D::Texture3D(LPCWSTR name)
{
	this->name = name;
}

Texture3D::Texture3D(LPCWSTR name, GraphicsDevice* device, DXGI_FORMAT format, int Width, int Height, int Depth)
{
	this->name = name;
	Create(device, format, Width, Height, Depth);
}

bool Texture3D::Create(GraphicsDevice* device, D3D12_RESOURCE_DESC Desc)
{
	desc = Desc;
	// create default heap
...
bool Texture3D::Create(GraphicsDevice* device, DXGI_FORMAT format, int Width, int Height, int Depth)
{
...
bool Texture3D::Create(GraphicsDevice* device, DXGI_FORMAT format, D3D12_RESOURCE_FLAGS flag, int Width, int Height, int Depth)
{
	D3D12_RESOURCE_DESC resDesc = {};
	resDesc.DepthOrArraySize = 1;
	resDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE3D;
...
bool Texture3D::SetData(GraphicsDevice* device, BYTE* rawtexdata, int imagesize, DescriptorHeap* descHeap, int heapindex)
{
	// create upload heap
	...
	texData.RowPitch = imagesize / (desc.DepthOrArraySize * desc.Height); // size of all our triangle vertex data
	texData.SlicePitch = imagesize / desc.DepthOrArraySize; // also the size of our triangle vertex data
...
void Texture3D::CreateView(GraphicsDevice* device, DescriptorHeap* descHeap, int heapindex)
{
